...
class _TimeRackHomeState extends State<TimeRackHome> {
  ...
  initPlatformState() async {
    ...
    setState(() {
      ...
      if (globals.prefs.getInt("inOrOut") == 0) {
        _punchText = 'Check Out';
      } else {
        _punchText = 'Check In';
      }
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        title: Text('Time Rack :' +
            ' ' +
            globals.prefs.getString("companyName") +
            ' ' +
            globals.prefs.getString("personName") +
            '(' +
            globals.prefs.getString("empCode") +
            ')'),
      ),
      body: Column(
        children: <Widget>[
...
// A capture is already pending, do nothing.
  ...
  punchAttendance() {
    ...
    FormData formData = new FormData.from({
      ...
      "location": _currentLocation["latitude"].toString() +
          "," +
          _currentLocation["longitude"].toString(),
      "file": new UploadFileInfo(new File(filePath),
          globals.prefs.getString("empCode") + '_' + timestamp() + '.jpg')
    });
    // Send FormData
      ...
      if (response.statusCode == 200) {
        _scaffoldKey.currentState.showSnackBar(new SnackBar(
          ...
          content: new Row(
            children: <Widget>[
              globals.prefs.getInt("inOrOut") == 0
                  ? new Text("Checked Out succesfully")
                  : new Text("Checked In succesfully")
            ],
          ),
        ));
        if (globals.prefs.getInt("inOrOut") == 0 ||
            globals.prefs.getInt("inOrOut") == null) {
        ...
        _scaffoldKey.currentState.showSnackBar(new SnackBar(
          duration: new Duration(seconds: 2),
          content: new Row(
            children: <Widget>[new Text("Some error occured")],
          ),
        ));
      }
    });
